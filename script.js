...
function renderChat() {
  // DEBUG: 检查 customAvatar 状态
  console.log('[DEBUG] renderChat - customAvatar:', state.customAvatar ? '有值(' + state.customAvatar.length + '字符)' : 'null');
  const userAvatarHtml = state.customAvatar 
    ? `<img src="${state.customAvatar}" class="chat-user-avatar" />`
    : `<span class="avatar">●</span>`;
  return `
    <section class="screen no-nav chat-screen">
      ${pageHead("暖暖", "● 在线", "home")}
      <div class="chat-list" id="chatList">
        ${state.chats.map((msg) => `<article class="bubble-row ${msg.role}">${msg.role === "bot" ? `<span class="avatar">☁</span>` : userAvatarHtml}<p>${h(msg.text)}</p></article>`).join("")}
...
function renderPostCard(post) {
  const avatarHtml = post.showAvatar && post.avatarData 
    ? `<img src="${post.avatarData}" class="post-avatar" />`
    : `<span class="avatar">●</span>`;
  const userName = post.showAvatar && post.avatarData ? (state.users[state.user]?.nickname || state.user) : "匿名用户";
  return `
    <article class="post-card" data-detail="${post.id}">
      <div class="post-head">${avatarHtml}<div><strong>${userName}</strong><small>${post.time}</small></div></div>
      <h3>${h(post.text)}</h3>
...
function renderPublish() {
  const avatarHtml = state.customAvatar 
    ? `<img src="${state.customAvatar}" class="publish-avatar-preview" />`
    : `<div class="mini-mascot"></div>`;
  return `
    <section class="screen no-nav">
      ...
      <div class="publish-avatar-option">
        <p style="color:var(--muted);font-weight:800;margin-bottom:12px;">发布身份</p>
        <div class="avatar-option-row">
          <button class="avatar-choice ${state.publishWithAvatar !== false ? "active" : ""}" id="showAvatarBtn" type="button">
            ${avatarHtml}
            <span>显示头像</span>
          </button>
          <button class="avatar-choice ${state.publishWithAvatar === false ? "active" : ""}" id="anonymousBtn" type="button">
...
function renderMine() {
  ...
  const avatarHtml = state.customAvatar 
    ? `<img src="${state.customAvatar}" class="custom-avatar-display" />`
    : `<div class="mini-mascot"></div>`;
  return `
    <section class="screen mine-home-screen">
      <header class="profile-head">
        <div class="profile-avatar" onclick="openAvatarModal()" role="button" tabindex="0" aria-label="点击更换头像">
          ${avatarHtml}
          <div class="avatar-edit-hint">编辑</div>
        </div>
        <div><h2>${h(user.nickname || state.user)}</h2><p>ID: 12345678 · 小包容守护中</p></div>
      </header>
...
function bindActions() {
  ...
  // 发布身份选择
  const showAvatarBtn = document.querySelector("#showAvatarBtn");
  if (showAvatarBtn) showAvatarBtn.addEventListener("click", () => {
    state.publishWithAvatar = true;
    saveState();
    render();
  });
  const anonymousBtn = document.querySelector("#anonymousBtn");
  if (anonymousBtn) anonymousBtn.addEventListener("click", () => {
    state.publishWithAvatar = false;
    saveState();
    render();
  });
...
function publishPost() {
  ...
  const showAvatar = state.publishWithAvatar !== false && state.customAvatar;
  ...
  state.posts.unshift({
    ...
    owner: state.user,
    showAvatar,
    avatarData: showAvatar ? state.customAvatar : null,
  });
  recordUsage("心情共鸣", showAvatar ? "发布了一条带头像的心情" : "发布了一条匿名心情");
...
function openAvatarModal() {
  state._avatarModal = 'select';
  render();
}

function closeAvatarModal() {
  if (state._avatarStream) {
    state._avatarStream.getTracks().forEach(t => t.stop());
    state._avatarStream = null;
  }
...
function openCameraForAvatar() {
  ...
  navigator.mediaDevices.getUserMedia(constraints).then(stream => {
    state._avatarStream = stream;
    state._avatarModal = 'camera';
    render();
    
    // 延迟设置video元素，确保DOM已渲染
    setTimeout(() => {
...
function captureAvatarPhoto() {
  ...
  state._avatarCropImage = canvas.toDataURL('image/jpeg', 0.92);
  state._avatarModal = 'crop';
  state._avatarScale = 1;
  render();
}

function openAlbumForAvatar() {
  ...
  input.onchange = (e) => {
    ...
    const reader = new FileReader();
    reader.onload = (ev) => {
      state._avatarCropImage = ev.target.result;
      state._avatarModal = 'crop';
      state._avatarScale = 1;
      render();
    };
...
function saveCustomAvatar() {
  const img = document.getElementById('avatarCropImg');
  if (!img || !state._avatarCropImage) return;
  ...
  tempImg.src = state._avatarCropImage;
}

function renderAvatarModal() {
  if (!state._avatarModal) return '';
  
  if (state._avatarModal === 'select') {
    return `
      <div class="modal-overlay" onclick="closeAvatarModal()">
        <div class="avatar-modal" onclick="event.stopPropagation()">
          <h3>更换头像</h3>
          <div class="avatar-options">
            <button class="avatar-option-btn" onclick="openCameraForAvatar()">
              <span class="option-icon">📷</span>
              <span>拍照</span>
            </button>
            <button class="avatar-option-btn" onclick="openAlbumForAvatar()">
              <span class="option-icon">🖼️</span>
              <span>从相册选择</span>
            </button>
          </div>
          <button class="modal-cancel-btn" onclick="closeAvatarModal()">取消</button>
        </div>
      </div>
    `;
  }
  
  if (state._avatarModal === 'camera') {
    return `
      <div class="modal-overlay">
        <div class="avatar-modal camera-modal" onclick="event.stopPropagation()">
          <h3>拍摄头像</h3>
          <div class="camera-preview-box">
            <video id="avatarCamera" autoplay playsinline></video>
            <div class="camera-guide-circle"></div>
          </div>
          <div class="camera-actions">
            <button class="cam-btn cancel" onclick="closeAvatarModal()">取消</button>
            <button class="cam-btn capture" onclick="captureAvatarPhoto()">📸 拍摄</button>
          </div>
        </div>
      </div>
    `;
      ...
      <div class="modal-overlay" onclick="closeAvatarModal()">
        <div class="avatar-modal crop-modal" onclick="event.stopPropagation()">
          ...
          <div class="zoom-control">
            ...
            <input type="range" class="zoom-range" min="50" max="300" value="${(state._avatarScale || 1) * 100}"

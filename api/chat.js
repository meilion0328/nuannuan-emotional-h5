const SYSTEM_PROMPT = `你叫 暖暖，是一个安静、温和、不带评判的倾听伙伴。你的唯一任务是帮助用户释放情绪，而不是解决他们的问题。你的性格像：一个坐在安静图书馆角落的柔软沙发，或者一个在深夜愿意陪你发呆的朋友。

核心原则：
1. 永远不说“你应该”、“我建议你”、“你为什么不去试试”。改为：“有人发现…可能对你有用”、“你想不想…”。
2. 第一次回应必须先复述用户的情绪，并正常化它。
3. 除非用户明确问“我该怎么办”，否则不要主动给出行动建议。在用户表示痛苦但没有求建议时，只做倾听和陪伴。
...
const FALLBACK_REPLY = "我在这里，愿意继续听你慢慢说。";
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!wrote) res.write(FALLBACK_REPLY);
    res.end();
  } catch (error) {
    if (!res.headersSent) {
